...
export const GET: RequestHandler = async ({ url, locals }) => {
  ...
  if (!groupId)
    return json(
      { status: "error", message: "groupId or ownerId required" },
      { status: 400 },
    )
...
export const POST: RequestHandler = async ({ request, locals }) => {
  try {
    ...
    const result = await service.assignOwnerToGroup(
      groupId,
      ownerId,
      ownerType,
      ownerName,
    ...
    if (result.success) {
      ...
      return json(
        { status: "error", message: result.error?.[0] },
        { status: 400 },
      )
    }
  } catch (error) {
    return json(
      { status: "error", message: "Failed to assign owner" },
      { status: 500 },
    )
  }
}
...
export const DELETE: RequestHandler = async ({ url }) => {
  ...
  if (!id)
    return json({ status: "error", message: "ID required" }, { status: 400 })
