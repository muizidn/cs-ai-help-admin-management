...
export const GET: RequestHandler = async ({ url, locals }) => {
  ...
  try {
    ...
    return json(
      { status: "error", message: "Internal server error", requestId },
      { status: 500 },
    )
  }
}
...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  try {
    ...
    const service = new PricingService()
    const result = await service.createPricingConfig(
      data,
      (locals as any).user?.email || "admin",
    )
...
export const PATCH: RequestHandler = async ({ request, url, locals }) => {
  ...
  try {
    const data: PricingUpdateInput = await request.json()
    const service = new PricingService()
    const result = await service.updatePricingConfig(
      id,
      data,
      (locals as any).user?.email || "admin",
    )
    ...
    if (result.success) {
      ...
      return json(
        { status: "error", message: result.error?.[0], requestId },
        { status: 400 },
      )
    }
  } catch (error) {
...
export const DELETE: RequestHandler = async ({ url, locals }) => {
  ...
  if (!id) {
    return json(
      { status: "error", message: "ID is required", requestId },
      { status: 400 },
    )
  }
