...
export const PATCH: RequestHandler = async ({ params, request, locals }) => {
  ...
  try {
    ...
    const flag =
      body.flag === null || body.flag === undefined ? null : String(body.flag)

    logger.info(
      { requestId, id, flag, type: "update_log_flag" },
      "Updating execution log flag",
    )
    ...
    if (success) {
      ...
      return json(
        { status: "error", message: "Failed to update flag or log not found." },
        { status: 400 },
      )
    }
  } catch (error) {
