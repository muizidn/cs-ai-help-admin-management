...
export const GET: RequestHandler = async ({ url, locals }) => {
  ...
  try {
    ...
    if (
      sortBy &&
      ["start_time", "end_time", "total_duration_ms", "created_at"].includes(
        sortBy,
      )
    ) {
    ...
    const finalDecision = url.searchParams.get("final_decision")
    if (finalDecision) {
      query.final_decision = finalDecision
    }

    const customerMessage = url.searchParams.get("customer_message")
    if (customerMessage) {
      query.customer_message = customerMessage
    }

    const aiResponse = url.searchParams.get("ai_response")
    if (aiResponse) {
