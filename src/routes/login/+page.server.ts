...
export const actions: Actions = {
  default: async ({ request, cookies, locals }) => {
    ...
    logger.info({ requestId, username, ip, ua }, "Admin login attempt")

    /* Rate limit */
    if (isRateLimited(ip)) {
      logger.warn({ requestId, ip }, "Admin login rate limited")
    ...
    if (!ADMIN_USERNAME || !ADMIN_PASSWORD_HASH_BASE64) {
      logger.error(
        { requestId },
        "ADMIN_USERNAME / ADMIN_PASSWORD_HASH_BASE64 not set",
      )
    ...
    if (!validPass) {
      logger.warn({ requestId, username, ip }, "Admin login failed")
    ...
    logger.info({ requestId, username, ip }, "Admin login successful")
