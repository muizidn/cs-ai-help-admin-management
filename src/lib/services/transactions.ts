...
export class TransactionService {
  ...
  private mapTransaction(transaction: any): Transaction {
    ...
    if (!type) {
      if (
        transaction.planId?.includes("pro_") ||
        transaction.planId === "pro"
      ) {
        type = "PLAN_UPGRADE"
      } else if (
        transaction.planId?.includes("credits_") ||
        transaction.planId === "credit"
      ) {
        type = "CREDIT_PURCHASE"
      } else if (
        transaction.notes?.toLowerCase().includes("manual credit update")
      ) {
        type = "CREDIT_UPDATE"
      } else {
    ...
    return {
      ...
      transactionCode:
        ...
        `TRX-${transaction.id?.substring(0, 8) || "UNK"}`,
  ...
  async getUsers(
    ...
    try {
        ...
        users.map(async (userDoc) => {
          ...
          return {
            ...
            billingSummary: {
              ...
              expiresAt: billingStateDoc?.currentPeriodEnd
                ? String(billingStateDoc.currentPeriodEnd)
                : undefined,
      ...
      const stats: TransactionStats = {
        ...
        billingPlans: {
          basic: enrichedUsers.filter((u) => u.billingSummary?.plan === "basic")
            .length,
          pro: enrichedUsers.filter((u) => u.billingSummary?.plan === "pro")
            .length,
          enterprise: enrichedUsers.filter(
            (u) => u.billingSummary?.plan === "enterprise",
          ).length,
        },
        billingStatus: {
          active: enrichedUsers.filter(
            (u) => u.billingSummary?.status === "active",
          ).length,
          expired: enrichedUsers.filter(
            (u) => u.billingSummary?.status === "expired",
          ).length,
          pending: enrichedUsers.filter(
            (u) => u.billingSummary?.status === "pending",
          ).length,
        },
        totalCredits: enrichedUsers.reduce(
          (sum, u) => sum + (u.billingSummary?.creditBalance || 0),
          0,
        ),
      }
  ...
  async updateUser(
    ...
    try {
      ...
      const { name, isActive, tags } = otherUpdates as any
      ...
      return {
        success: true,
        data: updatedUserDoc
          ? (this.mapUser(updatedUserDoc) as any)
          : undefined,
      }
    } catch (error) {
  ...
  async getTransactions(
    ...
    try {
      ...
      const voucherCodes = [
        ...new Set(transactions.map((t) => t.voucherCode).filter(Boolean)),
      ] as string[]
      ...
      const stats: TransactionStats = {
        ...
        byStatus: {
          completed: enrichedTransactions.filter(
            (t) => t.status === "COMPLETED",
          ).length,
          pending: enrichedTransactions.filter((t) => t.status === "PENDING")
            .length,
          failed: enrichedTransactions.filter((t) => t.status === "FAILED")
            .length,
        },
        byType: {
          ...
          planUpgrade: enrichedTransactions.filter(
            (t) => t.type === "PLAN_UPGRADE",
          ).length,
        },
  ...
  async updateTransaction(
    ...
    try {
      ...
      const currentTxDoc = await transactionsCollection.findOne({
        id: transactionId,
      })
      if (!currentTxDoc) {
        return {
      ...
      const isManualApproval =
        ...
        (updateData.metadata?.manuallyApproved ||
          currentTxDoc.metadata?.manuallyApproved)

      const callbackUrl =
        updateData.metadata?.manualCallbackUrl ||
        currentTxDoc.metadata?.manualCallbackUrl

      if (isManualApproval && callbackUrl) {
        logger.info(
          { transactionId, callbackUrl },
          "Manual approval detected with webhook. Triggering simulation.",
        )
        ...
        try {
          ...
          if (response.ok) {
            logger.info(
              { transactionId },
              "Manual approval webhook triggered successfully",
            )
            // The webhook should have processed the transaction and marked it as completed
            const updatedTxDoc = await transactionsCollection.findOne({
              id: transactionId,
            })
            return {
              success: true,
            ...
            logger.warn(
              { transactionId, status: response.status, errorText },
              "Manual approval webhook failed. Manual balance sync via ledger is restricted to the main app.",
            )
          }
        } catch (error) {
          logger.error(
            { transactionId, error: error as any },
            "Error triggering manual approval webhook, falling back to local approval",
          )
        }
      }
  ...
  private async applyTransactionSideEffects(transaction: Transaction) {
    ...
    if (creditsToAdd > 0) {
      const billingStatesCollection = await this.getCollection("billing-state")
      const currentState = await billingStatesCollection.findOne({
        ownerId,
        ownerType,
      })
      ...
      await callBillingWebhook({
        ...
        reason:
          transaction.reason ||
          `Approved transaction: ${transaction.transactionCode}`,
      })
    }
  }
  ...
  async updateUserCredit(
    ...
    try {
      ...
      if (!webhookSuccess) {
        logger.error(
          `Failed to sync manual credit update for user ${userId} to main app`,
        )
        return {
          success: false,
