...
export class PricingService {
    ...
    async getGlobalPricingConfig(): Promise<ServiceResult<PricingConfig>> {
        ...
        try {
            ...
            try {
                ...
                logger.warn("Redis error saving global pricing config:", redisError as any)
            }

            return { success: true, data: config }
        } catch (error) {
            logger.error({ error }, "Failed to get global pricing config")
            return { success: false, error: ["Failed to retrieve global pricing config"] }
        }
    }
    ...
    async getPricingConfigs(): Promise<ServiceResult<PricingConfig[]>> {
        try {
            const collection = await this.getCollection()
            const docs = await collection.find({ type: "pricing" }).sort({ updatedAt: -1 }).toArray()
    ...
    async getPricingConfigById(id: string): Promise<ServiceResult<PricingConfig>> {
        try {
            const collection = await this.getCollection()
    ...
    async createPricingConfig(input: Partial<PricingConfig>, user: string): Promise<ServiceResult<PricingConfig>> {
        try {
            const collection = await this.getCollection()
            const config = {
    ...
    async updatePricingConfig(id: string | null, input: PricingUpdateInput, user: string): Promise<ServiceResult<PricingConfig>> {
        try {
            const collection = await this.getCollection()

            const filter = id ? { _id: new ObjectId(id) } : { type: "pricing", isGlobal: true }
            const doc = await collection.findOne(filter)

            const currentConfig = doc ? { ...doc } : { ...this.getDefaultConfig(), type: "pricing", isGlobal: true }

            const newConfig = {
                ...currentConfig,
                ...input,
                expiresAt: input.expiresAt ? new Date(input.expiresAt) : ('expiresAt' in input ? null : currentConfig.expiresAt),
                proPlan: { ...(currentConfig as any).proPlan, ...(input.proPlan || {}) },
                credits: { ...(currentConfig as any).credits, ...(input.credits || {}) },
                usageCosts: { ...(currentConfig as any).usageCosts, ...(input.usageCosts || {}) },
                updatedAt: new Date(),
                updatedBy: user
            }
            ...
            return { success: true, data: { id: id || "global", ...newConfig } as any }
        } catch (error) {
    ...
    async deletePricingConfig(id: string): Promise<ServiceResult<boolean>> {
        try {
            ...
            if (config?.isGlobal) return { success: false, error: ["Cannot delete global pricing"] }
    ...
    async getGroupAssignments(groupId: string): Promise<ServiceResult<PricingGroupAssignment[]>> {
        try {
    ...
    async assignOwnerToGroup(groupId: string, ownerId: string, ownerType: string, ownerName: string, user: string): Promise<ServiceResult<PricingGroupAssignment>> {
        try {
            const collection = await this.getAssignmentCollection()

            const existing = await collection.findOne({ ownerId })
            if (existing) {
                return { success: false, error: ["Owner is already assigned to a group"] }
            }
    ...
    async searchOwnersGroup(ownerId: string): Promise<ServiceResult<string | null>> {
        try {
            const collection = await this.getAssignmentCollection()
            ...
            const group = await pricingCollection.findOne({ _id: new ObjectId(assignment.groupId) })
