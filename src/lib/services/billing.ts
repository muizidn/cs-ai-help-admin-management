...
export class BillingService {
    ...
    try {
      ...
      if (search) {
        ...
        filteredEntities = entities.filter(
          (e) =>
            searchRegex.test(e.name) || (e.email && searchRegex.test(e.email)),
        )
      }
      ...
      const paginatedItems = filteredEntities.slice(
        (page - 1) * limit,
        page * limit,
      )
  ...
  async updateBillingState(
    ...
    try {
      ...
      const ownerType =
        currentState.ownerType ||
        (ownerId.startsWith("org-") ? "organization" : "user")
