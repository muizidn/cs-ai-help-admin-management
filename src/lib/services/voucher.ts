...
export class VoucherService {
  ...
  async getVouchers(
    ...
    try {
      ...
      // Calculate stats
      const totalVouchers = await collection.countDocuments({
      ...
      const usageDocs = await collection
        .aggregate([
  ...
  async updateVoucher(
    ...
    try {
      ...
      if (processedData.expiresAt !== undefined) {
        processedData.expiresAt = processedData.expiresAt
          ? new Date(processedData.expiresAt)
          : null
      }
  ...
  async getVoucherTransactions(voucherId: string): Promise<ApiResponse<any[]>> {
    try {
      ...
      const transactions = await db
        ...
        .find({
          $or: [{ voucherId: voucherId }, { "metadata.voucherId": voucherId }],
        })
        .sort({ createdAt: -1 })
        .toArray()
